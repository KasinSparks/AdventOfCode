...
pub fn sln(input_path: &str) -> i64 {
    ...
    for line in input {
        ...
        result += nums.first().unwrap() - diff_loop(&nums);
    }
...
fn diff_loop(vals: &Vec<i64>) -> i64 {
    ...
    if !is_all_zeros {
        return *new_vals.first().unwrap() - diff_loop(&new_vals);
    }

    return *new_vals.first().unwrap();
}
...
mod tests {
    ...
    #[test]
    fn practice_result() {
        assert_eq!(sln("./src/days/day_09/practice_input.txt"), 2);
    }

    #[test]
    fn final_result() {
        assert_eq!(sln("./src/days/day_09/input.txt"), 971);
    }
}