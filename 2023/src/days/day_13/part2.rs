...
fn check_rows(map: &[String], rows: (usize, usize)) -> (bool, bool) {
    if map[rows.0] == map[rows.1] {
        return (true, false);
    }

    let mut diff_by_one = false;
    for i in 0..map[rows.0].len() {
        if map[rows.0].as_bytes()[i] != map[rows.1].as_bytes()[i] {
            if diff_by_one {
                return (false, false);
            } else {
                diff_by_one = true;
...
fn check_cols(map: &[String], cols: (usize, usize)) -> (bool, bool) {
    let diff_by_one = false;
...
mod tests {
    ...
    #[test]
    fn practice_result() {
        assert_eq!(sln("./src/days/day_13/practice_input.txt"), 400);
    }

    #[test]
    fn final_result() {
        assert_eq!(sln("./src/days/day_13/input.txt"), 0);
    }
}