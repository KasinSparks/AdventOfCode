...
class Cube():
    def __init__(self, x: int, y: int, z: int, w: int, val: int):
        self.x = x
        self.y = y
        self.z = z
        self.w = w
...
class Block():
    ...
    def add_cube(self, cube: Cube) -> None:
        _hash = self.__gen_hash__(cube.x, cube.y, cube.z, cube.w)
        ...
        self.__check_bounds__(cube.w, self.d4)
    ...
    def get_cube(self, x: int, y: int, z: int, w: int) -> Cube:
        _hash = self.__gen_hash__(x, y, z, w)
        try:
            return self.cubes[_hash]
        except KeyError as e:
            return None

    def __gen_hash__(self, x: int, y: int, z: int, w: int) -> str:
        return str(x) + ',' + str(y) + ',' + str(z) + ',' + str(w)
    
    def get_neighbors(self, x: int, y: int, z: int, w: int) -> list:
        neighbors = []
            ...
            for _y in range(-1, 2):
                for _z in range(-1, 2):
                    for _w in range(-1, 2):
                        ...
                        n = self.get_cube(x + _x, y + _y, z + _z, w + _w)
                        neighbors.append(n)
    ...
    def print_block(self):
        count = 0
        for w in range(self.d4[0], self.d4[1] + 1):
        ...
        for z in range(self.depth[0] - 1, self.depth[1] + 2):
            for y in range(self.height[0] - 1, self.height[1] + 2):
                for x in range(self.width[0] - 1, self.width[1] + 2):
                    for w in range(self.d4[0] - 1, self.d4[1] + 2):
                        c = self.get_cube(x, y, z, w)
...
def parse_input(input_lines: list) -> Block:
    ...
    for l in input_lines:
        ...
        for _l in line:
            if _l == '#':
                block.add_cube(Cube(x, y, 0, 0, _l))
            elif _l != '.':
