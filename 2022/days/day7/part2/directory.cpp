...
const aoc_directory *
aoc_directory::get_directory_to_delete(const uint64_t &needed_space) {
	const aoc_directory *smallest = this;
	std::cout << "test: " << smallest->_dir_name << std::endl;
	
	for (std::pair<std::string, aoc_directory> dir : this->_sub_dirs) {
		uint64_t temp_size = dir.second.calculate_total_size();
		if (temp_size < smallest->calculate_total_size() &&
				temp_size > needed_space) {
			smallest = this->get_directory(dir.first)->get_directory_to_delete(needed_space);
		}
		std::cout << "test2: " << smallest->_dir_name << std::endl;
	}
	
	/*
	for (std::pair<std::string, aoc_directory> dir : this->_sub_dirs) {
		uint64_t temp_size = dir.second.get_directory_to_delete(needed_space)->calculate_total_size();
		if (smallest->calculate_total_size() > temp_size && temp_size > needed_space) {
			smallest = this->get_directory(dir.first);
		}
		std::cout << "test3: " << smallest->_dir_name << std::endl;
	}

	std::cout << "test4: " << smallest->_dir_name << std::endl;
	*/
	return smallest;
}
