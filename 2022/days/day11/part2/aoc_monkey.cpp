...
void
aoc_monkey::run(std::list<aoc_monkey_thrown_item> *thrown_items, uint64_t lcm) {
	uint64_t new_worry_level = 0;

	while (!this->_items.empty()) {
		uint64_t item = this->_items.front();
		// Inspect item
		switch (this->_op) {
			case aoc_monkey_operation::ADD:
		...
		std::cout << "NWL: " << new_worry_level << ", ITEM: " << item << std::endl;
...
void
aoc_monkey::add(const uint64_t &item_id) {
