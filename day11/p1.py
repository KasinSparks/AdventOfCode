...
def main(testFile: bool):
    ...
    print("seats: ", count_seats(_seats))
    ...
    for j in range(len(seats)):
        for i in range(len(seats[j])):
            ...
            if val == ".":
                ...
                sim_seats[j][i] = "#"
            elif val == "#" and check_seat(seats, i, j) >= 4:
                sim_seats[j][i] = "L"
...
def check_los(seats, i, j):
    l1 = check_line(seats, i, j, 1, 0) and check_line(seats, i, j, -1, 0)
    l2 = check_line(seats, i, j, 0, 1) and check_line(seats, i, j, 0, -1)
    l3 = check_line(seats, i, j, 1, 1) and check_line(seats, i, j, -1, 1)
    l4 = check_line(seats, i, j, 1, 1) and check_line(seats, i, j, 1, -1)

    good_los = l1 and l2 and l3 and l4
    return good_los
